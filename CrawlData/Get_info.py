...
def get_profile_info_24(driver, url):
    try:
        ...
        title = get_job_title(page_source)
        date = get_posted_date(page_source)
        ...
        return [title, date, salary, description, src_pic]
    except Exception as e:
...
def get_vieclam24(driver, num_pages):
    try:
        ...
        while page_start <= num_pages:
            url = f'https://vieclam24h.vn/tim-kiem-viec-lam-nhanh?page={page_start}&q=it&sort_q='
            print('>>>URL', url)
            driver.get(url)
            sleep(2)
